...
use plotters::style::full_palette::{ORANGE, PURPLE, GREY};
...
fn main() -> Result<()> {
    ...
    let output_path = args.output.unwrap_or_else(|| input_path.with_extension("png"));
    
    if output_path.exists() && !overwrite {
        anyhow::bail!("Output file {:?} already exists. Use --overwrite to replace.", output_path);
    }
    ...
    // 3. PCA
    // n_components = 2
    let pca = PCA::fit(&scaled_matrix, PCAParameters::default().with_n_components(2))
        .map_err(|e| anyhow::anyhow!("PCA fit error: {}", e))?;
    
    let transformed: DenseMatrix<f32> = pca.transform(&scaled_matrix)
