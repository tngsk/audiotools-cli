...
use audiotools_core::dsp::{Stft, spectral_flux};
...
impl AudioSegmenter {
    ...
    fn detect_onsets(&self, y: &[f32], sr: u32) -> Vec<usize> {
        ...
        let variance: f32 = flux.iter().map(|x| (x - mean_flux).powi(2)).sum::<f32>() / flux.len() as f32;
    ...
    pub fn save_segments(&self, segments: &[Vec<f32>], sr: u32, original_path: &Path, output_dir: &Path, overwrite: bool) -> Result<()> {
...
mod tests {
    ...
    fn test_trim_silence() {
        ...
        assert!(start > 0, "Start {} should be > 0 (silence at start)", start);
    }
